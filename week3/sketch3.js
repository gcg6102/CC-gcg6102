  ...
  createCanvas(800,600);
    ...
    function draw(){
      ...
      for(let i = 0; i < 25; i++){  //code for cartesian in class but rectangle
        for(y = 0; y < 10; y++){
        circle(width/2,height/2, (i*10+2)*y);
        ellipseMode(circle(width/2,height/2, i*10+2));
      }
    }
      for(let i = 0; i < 25; i++){
        circle(mouseX,mouseY, i*10+2); //code for cartesian in class but rectangle
    ...
    rect(s*85, i*55, i * 4, i * 5*400); //it kinda looks like low poly frutiger towers omg
    circle(s*80, i*60, i * 4, i*10); 
    fill('lightblue'); 
    //rotate(65);
  }
    line(i-100,i*100,800,i+10);
    line(i-100/2,i*100/2,800,i+10);
    //
  for(y = 0; y < 10; y++){
